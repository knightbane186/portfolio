"use client";
import Head from "next/head";
import {BsFillMoonStarsFill} from 'react-icons/bs';
import {AiFillGithub, AiFillLinkedin} from 'react-icons/ai';
import Image from "next/image";
import deved from "../public/dev-ed-wave.png";
import design from '../public/design.png';
import code from '../public/code.png';
import consulting from '../public/consulting.png';
import weather from '../public/weather.png';
import hacker_news from '../public/hacker_news.png';
import messenger from '../public/messenger.png';
import movies from '../public/movies.png';
import voiceAI from '../public/voiceAI.png';
import adventure from '../public/adventure.png';
import { useState } from "react";
export default function Home() {
  const [darkMode, setDarkMode] = useState(false);
  return (
    <div className={darkMode ? "dark": ""}>
      <Head>
        <title>Denvers Space</title>
        <meta name="description" content="Generated by  cred" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <main className="bg-white px-10 md:px-20 lg:px-40  dark:bg-gray-900">
        <section className=" min-h-screen">
<nav className="py-10 mb-12 flex justify-between">
  <h1 className="text-xl font-burtons">developed by Denver</h1>
  <ul className="flex items-center">
    <li>
      <BsFillMoonStarsFill onClick={() => setDarkMode(!darkMode)} className="cursor-pointer text-xl"/>
    </li>
    <li><a className="bg-gradient-to-r from-cyan-500 to-teal-500 text-white px-4 py-2 rounded-md ml-8" href="/Full_Stack_Resume.pdf" target="_blank" rel="noopener noreferrer">
      Resume
    </a></li>
  </ul>
</nav>
<div className="text-center p-10">
  <h2 className="text-5xl py-2 text-teal-600 font-medium md:text-6xl">Denver Lopes</h2>
  <h3 className="text-2xl py-2 md: text-3xl dark: text-gray-400">Full-Stack Developer.</h3>
  <p className="text-md py-5 leading-8 text-gray-800 md:text-xl max-w-lg mx-auto dark:text-gray-300">
    A self-taught developer w/ pro skills in web & mobile applications development.
    Check out my projects down below and take it from there!
  </p>
</div>
<div className="text-5xl flex justify-center gap-16 py-3 text-gray-600">
<a  className =" dark:text-purple-400"  href="https://github.com/knightbane186" target="_blank" rel="noopener noreferrer">
    <AiFillGithub />
  </a>
  <a className=" dark:text-blue-700" href="https://www.linkedin.com/in/denver-lopes-3a7848193/" target="_blank" rel="noopener noreferrer">
    <AiFillLinkedin />
  </a>


</div>
<div className="relative bg-gradient-to-b from-teal-500 rounded-full w-80 h-80 mt-20 mx-auto overflow-hidden md:h-96 md:w-96">
  {/* now some  comps below are Next.js specific */}
  <Image src={deved}  layout="fill" objectFit="cover"/>
</div>
        </section>

<section>
  <div>
    <h3 className="text-3xl py-1 dark:text-gray-400">My skills package</h3>
    <p className="text-md py-2 leading-8 text-gray-800 dark:text-gray-200">With an extensive skillset spanning TypeScript, React Native, Swift, JavaScript, HTML & CSS, Python, Firebase, and more, I am excited about the opportunity to contribute my technical proficiency to your team.
Currently serving as a Full Stack Developer, I am engaged in the migration of an iOS app to a cross-platform framework using React Native, Expo CLI, Firebase, TypeScript, and Tailwind CSS. My role involves crafting architectural designs, implementing key features, and ensuring an optimal user experience.</p>
    <p className="text-md py-2 leading-8 text-gray-800 dark:text-gray-200">I excel at utilizing Expo CLI for streamlined development setup and Firebase for efficient backend functionality. TypeScript enhances code quality, and Tailwind CSS ensures a consistent UI design. Collaborating closely with diverse teams, I bring projects to fruition, ensuring alignment with user needs and timelines.
I founded Cuppastreet, a start-up centered around crafting mugs from reclaimed timber, demonstrating my entrepreneurial spirit. With unwavering commitment, I navigated challenges, including the tumultuous year of 2020, while maintaining my restaurant job.
I am excited about the opportunity to discuss how my technical expertise and dynamic experiences align with your company’s vision. Thank you for your consideration. </p>
  </div>
  <div className="lg:flex gap-10">
    <div className="text-center shadow-lg p-10 rounded-xl my-10">
      <div className="flex justify-center">
      <Image src={design} width={100} height={100}/>
      </div>
     
      <h3 className="text-lg font-medium pt-8 pb-2 dark:text-gray-400">Front End </h3>
      <p className="dark:text-gray-100">With extensive experience in front-end development, I specialize in crafting intuitive user interfaces using technologies like React Native, TypeScript, and Tailwind CSS. My expertise lies in creating seamless user experiences and ensuring high-quality code for optimal app performance.</p>
      <h4 className="py-4 text-teal-600">Tools I use</h4>
      <p className="text-gray-800 py-1 dark:text-gray-100">VS Code, Atom</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">React, ReactNative, Angular, Vue.js</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Next.js, Bootstrap, Material Design</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Lighthouse, GTmetrix</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Figma, Adobe XD, InVision, Blender</p>
    </div>
    <div className="text-center shadow-lg p-10 rounded-xl my-10">

<div className="flex justify-center">
<Image src={code} width={100} height={100}/>

</div>
      
      <h3 className="text-lg font-medium pt-8 pb-2 dark:text-gray-400">Back End</h3>
      <p className="dark:text-gray-100">In backend development, I excel at architecting robust solutions using technologies like Node.js, MongoDB, and Firebase. My proficiency extends to crafting efficient database structures and implementing secure authentication systems. With a keen eye for detail, I ensure backend operations run smoothly, enabling seamless user experiences.</p>
      <h4 className="py-4 text-teal-600">Tools I use</h4>
      <p className="text-gray-800 py-1 dark:text-gray-100">MySQl, Node.js, MongoDB, Oracle, DynamoDB</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Apache, AWS Elastic Beanstalk, Firebase </p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Docker, Kubernetes, Gitlab </p>
    </div>
    <div className="text-center shadow-lg p-10 rounded-xl my-10">
      <div className="flex justify-center">  
       <Image src={consulting} width={100} height={100}/>
      </div>
      <h3 className="text-lg font-medium pt-8 pb-2 dark:text-gray-400">Team Work handlers</h3>
      <p className="dark:text-gray-100"> This ensures that our team consistently delivers high-quality work, meeting objectives with precision and efficiency. My proactive approach and adaptability make me an invaluable asset in any collaborative setting.</p>
      <h4 className="py-4 text-teal-600">Tools I use</h4>
      <p className="text-gray-800 py-1 dark:text-gray-100 ">Confluence, Jira, Asana</p>
      <p className="text-gray-800 py-1 dark:text-gray-100"> Trello, Slack</p>
      <p className="text-gray-800 py-1 dark:text-gray-100">Github, Gogs, SourceForge</p>
    </div>
  </div>
</section>
<section>
  <div>
    <h3 className="text-3xl py-1 dark:text-gray-400">Portfolio</h3>
    <p className="text-md py-2 leading-8 text-gray-800 dark:text-gray-100"> Welcome! Explore my portfolio showcasing a diverse range of projects. From a Chat App with ChatGPT and DALL-E integration to an Adventure Network app, a Climate Assistance tool, and more. Dive in and discover my innovative creations.</p>
    <p className="text-md py-2 leading-8 text-gray-800 dark:text-gray-100">Click on the portfolios to visit the github repos</p>
   
  </div>
  <div className="flex flex-col gap-10 py-10 lg:flex-row lg: flex-wrap ">
    
    {/* <div className="basis-1/3 flex-1"><Image src={weather} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" /></div> */}

    <div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/Klim8">
    <Image src={weather} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>

<div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/Hacker-news-SwiftUI-">
    <Image src={hacker_news} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>


<div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/MessengerApp">
    <Image src={messenger} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>


<div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/Movies_browserApp">
    <Image src={movies} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>


<div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/Voice_assistant">
    <Image src={voiceAI} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>


<div className="basis-1/3 flex-1">
  <a href="https://github.com/knightbane186/travel_Adventure">
    <Image src={adventure} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" />
  </a>
</div>


















{/* 
   
    <div className="basis-1/3 flex-1"><Image src={hacker_news}className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive" /></div>
   
    <div className="basis-1/3 flex-1"><Image src={messenger} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive"/></div>

    <div className="basis-1/3 flex-1"><Image src={movies} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive"/></div>

    <div className="basis-1/3 flex-1"><Image src={voiceAI} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive"/></div>

    <div className="basis-1/3 flex-1"><Image src={web6} className="rounded-lg object-cover" width={'100%'} height={'100%'} layout="responsive"/></div> */}
  </div>
</section>
      </main>
    </div>
  );
}
